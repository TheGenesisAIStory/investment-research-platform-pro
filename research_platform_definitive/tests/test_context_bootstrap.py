...
def test_legacy_ml_stock_lab_exposes_workstation_runners() -> None:
    import ml_stock_lab
    from ml_stock_lab.factor_registry import FACTOR_BLOCKS
    from ml_stock_lab.training import train_ml_model_suite

    assert hasattr(ml_stock_lab, "run_ml_stock_lab_experiment")
    assert ml_stock_lab.run_ml_stock_lab_experiment is not None
    assert train_ml_model_suite is not None
    assert "value" in FACTOR_BLOCKS
