...
| --- | --- | --- | --- | --- |
| commodity | 9 | 9 | global | OK |
| crypto | 12 | 12 | global, us | OK |
| etf_commodity | 8 | 8 | global | OK |
| etf_equity | 33 | 33 | eu, global, it, us | OK |
| etf_fi | 17 | 17 | eu, global, it, us | OK |
| fx | 13 | 13 | eu, global | OK |
| yield_proxy | 3 | 3 | us | OK |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MKT_RF | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| SMB | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| HML | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| RMW | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| CMA | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| MOM | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| QMJ | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| BAB | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
| HML_DEVIL | not observed | no | gap | proxy/gap | gap | gap | gap | gap |
...
**Test gaps:** __init__.py, aqr_data.py, aqr_factors.py, banking_data.py, batch_downloader.py, data_utils.py, equity_feature_engineering.py, export_utils.py, fx_commodities.py, html_utils.py, market_universe.py, portfolio_analytics.py, storage_policy.py, valuation_analytics.py
...
### 2. Data Gaps
- FF/AQR international factors are not yet complete by region in a first-class artifact store.
...
### 3. UI Gaps
- Academic Reference page is either absent or not yet wired to FF/AQR factor return artifacts in this workspace.
...
- Feature metadata has broad coverage, but not every feature has formula_latex/source DOI/economic rationale fields.
- `FEATURE_ACADEMIC_REFERENCE.md` should become the canonical paper/formula reference and feed the UI.


### 5. Test Gaps
- Add tests for provider suffix normalization, FF factor downloads with mocked fallback, local Italy factor construction, feature metadata completeness, and Academic Reference rendering.
