...
- Files under `research_platform_definitive/`: 14,503
- Core Python modules inspected: 52
- Streamlit/app entries inspected: 17
- Pytest files inspected: 20
- Docs files: 31
- Output/artifact files: 13,426
- Feature metadata entries: 109
- Metric metadata entries: 49


**State:** v1 production-grade research workstation with several v2 layers already present; still needs-work for academic factor regional completeness, Asia single-name parsing, and metadata/formula completeness.
...
**Top critical issues:** dirty split between current workspace and target branch, some modules are implemented but not exposed fully in UI, and metadata contains features whose calculation/point-in-time implementation needs stricter cross-checks.
...
| --- | --- | --- | --- |
| output/data_completion/ResearchDataCoverageValidation.csv | 11 | {'OK': 7, 'PLANNED': 4} | 2739 |
| output/macro_market/tables/MacroAssetManifest.csv | 95 | {'OK': 95} | 28357 |
| output/macro_market/tables/MultiAssetUniverseManifest.csv | 95 | {'OK': 95} | 29828 |
| output/ml_stock_lab/MLStockLab_status.json |  |  | 111 |
| output/multi_asset_universe/MultiAssetUniverseManifest.csv | 95 | {'OK': 95} | 27345 |
